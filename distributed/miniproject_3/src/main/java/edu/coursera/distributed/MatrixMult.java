...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        final int endRow = Math.min((rank + 1) * rowChunk, nrows);
        ...
        if(rank==0){
            ...
            for (int i=1; i<size; i++){
                ...
                final int nElement = (rankEndRow-rankStartRow)* c.getNCols();
                requests[i-1]  = mpi.MPI_Irecv(c.getValues(),rowOffset,nElement,i,i, mpi.MPI_COMM_WORLD);

            }
            mpi.MPI_Waitall(requests);
        }
        else{
            mpi.MPI_Send(c.getValues(), startRow*c.getNCols(),(endRow-startRow)* b.getNCols(),0,rank,mpi.MPI_COMM_WORLD);
        }
    }
}
